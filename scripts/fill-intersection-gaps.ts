...
const TARGETS: CafeTarget[] = [
  { cafeName: "육아 돌봄수첩", cafeId: "31754837", ownerAccountId: "ahffkdlek12", keywords: ["장바구니캐리어", "접이식카트", "천안웨딩홀"] },
  { cafeName: "건강 체크노트", cafeId: "31754869", ownerAccountId: "ahffkekd12", keywords: ["청소업체추천", "해충방역업체"] },
  { cafeName: "건강 정보노트", cafeId: "31754875", ownerAccountId: "ahsxkfldk12", keywords: ["헤어드라이기"] },
  { cafeName: "생활 살림노트", cafeId: "31754939", ownerAccountId: "alsrudgus531", keywords: ["헤어드라이어"] },
  { cafeName: "일상 소소담", cafeId: "31755069", ownerAccountId: "pixelninja3", keywords: ["헤어에센스추천", "밀크씨슬", "방역업체"] },
];
...
const runTarget = async (target: CafeTarget): Promise<{ name: string; success: number; fail: number }> => {
  console.log(`\n=== ${target.cafeName} 갭 메우기 시작 (${target.keywords.length}개) ===`);
  ...
  console.log(`=== ${target.cafeName} 갭 메우기 완료: 성공 ${success} / 실패 ${fail} ===`);

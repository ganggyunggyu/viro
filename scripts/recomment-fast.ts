...
const TARGETS: Array<{ cafe: string; articleId: number }> = [
  ...[23, 22, 21, 20, 19, 18, 17, 16, 15, 14, 13, 12, 11, 9, 3].map((articleId) => ({ cafe: "육아 돌봄수첩", articleId })),
...
const main = async (): Promise<void> => {
  ...
  for (const [cafeName, articleIds] of byCafe) {
  ...
  logLine(
    `\n=== 전체 완료: 글 ${totalArticles}개 / 댓글삭제 ${totalDeleted}/${totalSeen} / 새댓글작성 ${totalPosted} ===`
  );

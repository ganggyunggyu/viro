...
import { Account, PublishedArticle, User, addCommentToArticle } from '../src/shared/models';
...
const callDeepSeek = async (messages: ChatMessage[]): Promise<{
  message: ChatMessage;
  finishReason: string;
}> => {
...
const main = async (): Promise<void> => {
  ...
  const executeTool = async (name: string, argsJson: string): Promise<string> => {
    ...
    if (name === 'list_accounts') {
      ...
      const existing = await PublishedArticle.findOne({ cafeId: CAFE_ID, articleId: ARTICLE_ID }, { comments: 1 }).lean<{
      ...
      return JSON.stringify(
        sorted.slice(0, limit).map((a) => ({ accountId: a.accountId, nickname: a.nickname })),
      );
    }
    ...
    if (name === 'post_comment') {
      ...
      const already = await hasCommented(CAFE_ID, ARTICLE_ID, accountId, 'comment');
      ...
      if (!result.success) {
        console.log(`[TOOL post_comment] FAIL ${accountId}: ${result.error}`);
      ...
      console.log(`[TOOL post_comment] SUCCESS ${accountId} commentId=${result.commentId}`);
    ...
    if (name === 'wait_minutes') {
      ...
      console.log(`[TOOL wait_minutes] ${minutes}분 대기`);
  ...
  const messages: ChatMessage[] = [
    ...
    {
      role: 'user',
      content: `카페 슬러그: ${CAFE_SLUG}, cafeId: ${CAFE_ID}, articleId: ${ARTICLE_ID} 글에 댓글 작업을 진행해줘.`,
    },
  ];
  ...
  while (turns < MAX_TURNS) {
    turns += 1;
    const { message, finishReason } = await callDeepSeek(messages);
    messages.push(message);

    if (!message.tool_calls || message.tool_calls.length === 0) {
      console.log(`[AGENT] 텍스트 응답 (도구 호출 없음, finish_reason=${finishReason}):`, message.content);
      break;
    }
    ...
    for (const toolCall of message.tool_calls) {
      ...
      console.log(`[AGENT] tool_call: ${name}(${argsJson})`);
      const result = await executeTool(name, argsJson);
      messages.push({ role: 'tool', tool_call_id: toolCall.id, content: result });

      if (name === 'finish') {
        console.log('[AGENT] finish 호출됨:', JSON.parse(argsJson).summary);
        shouldStop = true;
      }
    }
  ...
  if (turns >= MAX_TURNS) {
    console.log('[AGENT] 최대 턴 수 도달, 강제 종료');
  }

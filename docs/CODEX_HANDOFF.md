...
## WP-HARDENING-2 (WP-2/3/4 통합 적대 리뷰 발견 — 후속 수정)

실제 tsx 재현 기반. 우선순위:

- **F1 (high, WP-2)**: `cafe-account-policy.ts:43` targetCafeIds는 toCafeSlug 결과(슬러그, 예 'mealtalkdht')로 저장되는데 런타임 호출부는 숫자 cafe.cafeId(예 '31750114')를 넘겨 `includes` 매칭 실패 → 대상카페 지정 계정이 전 카페에서 제외(발행 중단 회귀). **수정**: 식별자 공간 통일. isEligibleForCafe(account, cafeId, cafeSlug)로 확장하고 targetCafeIds가 cafeId '또는' slug 를 포함하면 eligible. 호출부(batch-job/batch-queue/manual-publish/manuscript-actions/scripts)가 cafe.cafeId 와 함께 toCafeSlug(cafe.cafeUrl)도 넘기게. 하위호환(빈 targetCafeIds=전역) 유지.
- **F6a (medium, WP-2 test)**: `cafe-account-policy.test.ts`가 'A'/'B'(슬러그=cafeId) 퇴화값만 써서 F1을 은폐. **수정**: targetCafeIds=['mealtalkdht'](슬러그) 저장 + cafeId '31750114' + slug 'mealtalkdht' 로 조회 시 포함(positive), 타 카페 제외를 실값으로 고정. 이 테스트는 F1 수정 전엔 실패해야 함.
- **F2 (medium, WP-3)**: stub 제외 필터가 노출/verify 두 경로에만 있고 `modify-query-utils.ts:12 buildBaseFilter` 와 auto-comment 대상 쿼리(features/auto-comment .../actions.ts 의 status:'published' find 3곳)엔 없음 → 외부 stub 이 수정/자동댓글 대상으로 새나감. **수정**: 그 경로들에도 isExternal:{$ne:true} 추가(공용 헬퍼 권장).
- **F3 (medium, WP-4)**: `reply-handler.ts:228-238` 작성 성공 후 replyKey 영속화 실패가 삼켜져 job 이 dedup 레코드 없이 success 로 기록 → 재시도 시 중복 대댓글. **수정**: 영속화 실패를 성공으로 뭉개지 말고 return {success:false, willRetry:true}(또는 replyKey 를 작성 성공과 원자적으로/durable 하게). 이 경로를 밟는 테스트 추가.
- **F5 (low, WP-4)**: reply 멱등키가 accountId 무관 → 다른 계정의 동일 fallback 대댓글이 스킵됨. **수정**: 멱등키에 accountId 포함(retry=같은 계정은 여전히 dedup, 다른 계정은 허용). 테스트.
- **F4 (medium, WP-3)**: `backfill-external-article-stubs.ts:42` writerAccountId==='' 단독으로 external 판정 → 빈 writer 진짜 발행글을 파괴적으로 숨김. **수정**: 완전 stub 시그니처(title==='외부 글' && menuId==='' && keyword==='' && content==='') 요구. 테스트 갱신.
- **F6b (low, WP-3 test)**: 이미 존재하는 isExternal:false 발행글에 addCommentToArticle 후 isExternal 이 false 유지($setOnInsert 비덮어쓰기)를 고정하는 테스트 추가.

DoD: 위 fake-green 테스트(F6a, F6b, F3 경로)가 수정 전 실패→수정 후 통과. npm run test:harness/tsc/next build 그린 유지.

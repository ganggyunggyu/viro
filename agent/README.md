...
자체 운영 화면과 Playwright/Chrome 실행부를 함께 제공하는 데스크톱 프로그램이다.
앱은 원격 웹페이지를 감싸지 않고 패키지에 포함된 화면을 `file://`로 연다. Vercel은
계정·카페 데이터, AI 원고 준비, 작업 결과 동기화 API만 맡는다.

```
[데스크톱 로컬 UI] 작업 실행 → 브로커에서 데이터 준비 → 로컬 Playwright 실행 → 결과 동기화
```
...
앱은 홈, 글 발행·수정, 댓글, 노출 확인, 계정 점검, 카페 관리, 재작성, 로그, 연결 설정을
포함한 자체 UI를 연다. 다운로드 페이지에서 발급한 연결 토큰을 앱의 연결 설정에 한 번
저장하면 데이터를 불러오고, 저장된 토큰이 있으면 다음 실행부터 로컬 실행부도 자동 시작한다.
...
- `electron/` — 번들된 Viro 운영 UI, 연결 상태/로그, 앱 패키징

...
네이버 카페에 글, 댓글, 대댓글을 자동으로 발행하는 바이럴 마케팅 자동화 플랫폼. 저장소 이름은 `cafe-bot`이지만 서비스 이름은 바이로(VIRO)다. Next.js 대시보드에서 계정, 카페, 키워드를 관리하면 Playwright가 실제 브라우저로 네이버 카페에 로그인해 글을 쓰고, BullMQ 큐가 댓글/대댓글을 사람이 쓰는 것처럼 시간 간격을 두고 순서대로 발행한다.
